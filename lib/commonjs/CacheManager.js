...
class CacheEntry {
  ...
  async download(path) {
    ...
    if (source != null) {
      try {
        const result = await retry(() => _reactNativeFileAccess.FileSystem.fetch(source, {
          path,
          ...options
        }));
...
class CacheManager {
  ...
  static async clearCache() {
    if (await _reactNativeFileAccess.FileSystem.exists(CacheManager.config.baseDir)) {
      ...
      while (overflowSize > 0 && files.length) {
        ...
        if (file) {
          if (await _reactNativeFileAccess.FileSystem.exists(file.path)) {
            ...
            await _reactNativeFileAccess.FileSystem.unlink(file.path).catch(e => {
              if (__DEV__) {
                console.log(e);
              }
            });
