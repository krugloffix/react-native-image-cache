...
var _defaultConfiguration = _interopRequireDefault(
...
async function retry(fn) {
  let retriesLeft =
    arguments.length > 1 && arguments[1] !== undefined
      ? arguments[1]
      : CacheManager.config.maxRetries || 0;
  let interval =
    arguments.length > 2 && arguments[2] !== undefined
      ? arguments[2]
      : CacheManager.config.retryDelay;
  try {
    ...
    if (source != null) {
      try {
        const result = await retry(() =>
          _reactNativeFileAccess.FileSystem.fetch(source, {
            path,
            ...options,
          })
        );
...
class CacheManager {
  static defaultConfig = _defaultConfiguration.default;
  get config() {
  ...
  static get(source, options, noCache, maxAge) {
    ...
    if (
      ...
      ((_CacheManager$entries = CacheManager.entries[source].options) ===
        null ||
      _CacheManager$entries === void 0 ||
      (_CacheManager$entries = _CacheManager$entries.headers) === null ||
      _CacheManager$entries === void 0
        ? void 0
        : _CacheManager$entries.Authorization) !==
        (options === null ||
        options === void 0 ||
  ...
  static async clearCache() {
    if (
      await _reactNativeFileAccess.FileSystem.exists(
        CacheManager.config.baseDir
      )
    ) {
      const files = await _reactNativeFileAccess.FileSystem.ls(
        CacheManager.config.baseDir
      );
      for (const file of files) {
        try {
          await _reactNativeFileAccess.FileSystem.unlink(
            `${CacheManager.config.baseDir}${file}`
          );
        } catch (e) {
          if (__DEV__) {
  ...
  static async getCacheSize() {
    const result = await _reactNativeFileAccess.FileSystem.stat(
      CacheManager.config.baseDir
    );
    if (!result) {
  ...
  static async prefetchBlob(source, options) {
    ...
    if (path) {
      const blob = await _reactNativeFileAccess.FileSystem.readFile(
        path,
        'base64'
      );
      return blob;
  ...
  static async pruneCache() {
    ...
    const files = await _reactNativeFileAccess.FileSystem.statDir(
      CacheManager.config.baseDir
    );
    files.sort((a, b) => {
      ...
      while (overflowSize > 0 && files.length) {
        ...
        if (file) {
          if (await _reactNativeFileAccess.FileSystem.exists(file.path)) {
            ...
            await _reactNativeFileAccess.FileSystem.unlink(file.path).catch(
              e => {
                if (__DEV__) {
                  console.log(e);
                }
...
const getCacheEntry = async (cacheKey, maxAge) => {
  ...
  if (maxAge && exists) {
    const { lastModified } = await _reactNativeFileAccess.FileSystem.stat(path);
